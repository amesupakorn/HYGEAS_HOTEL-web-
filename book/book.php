...
<head>
	...
	<?php 
        ...
        if($_SERVER["REQUEST_METHOD"] === "POST"){
			...
			require('checkBooking.php');
			$conn = new database;
			
			$sqlevent = "SELECT eCode, eChoice, eDiscount, eGuests, eFor, eMin, eTypeRoom
					FROM `Event`;";
			$CheckEvent = mysqli_query($conn->getDatabase(), $sqlevent);
			
			$objBook = new CheckBooking($conn, $_POST['checkin'], $_POST['checkout'], $_POST['people'], $_POST['type'], $_SESSION['user_login']);
			$check = $objBook->checkTable();
			if($check == 1){
                ...
                echo "Swal.fire({
                    position: 'center',
                    icon: 'error',
                    title: 'The room is full at this time.',
				...
				$objPayment = new Payment($objBook->getPay(), $objBook->getType(), $objBook->getNight());
				$objPayment->showPayment($objBook->createCommand());
			}
	...
	<?php
		...
		// 	if(isset($_SESSION['code']) == 'fail'){
		// 		$_SESSION['code'] = "finish";
        //         echo '<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>"';
        //         echo "<script>";
        //         echo "Swal.fire({
        //             position: 'center',
        //             icon: 'error',
        //             title: 'There is no this code',
        //             showConfirmButton: false,
        //             timer: 1500
        //         })";
        //         echo "</script>";
	    ...
	    if(isset($_SESSION['pass'])){
            if($_SESSION['pass'] == "failpay"){
                $_SESSION['pass'] = "finish";
                echo '<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>"';
                echo "<script>";
                echo "Swal.fire({
                    position: 'center',
		...
		if(isset($_SESSION['refund'])){
			if($_SESSION['refund'] == "complete"){
				$_SESSION['refund'] = "finish";
				echo '<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>"';
                echo "<script>";
                echo "Swal.fire({
                    position: 'center',
	...
	<link rel="stylesheet" href="../payment/styles/examples.css">
...
<?php 
  class Payment{
    private $type, $classCss;
    private $money, $pernight, $perrsonMin, $personMax;
	private $night;
    public static $public_key = 'pkey_test_5x48zalcuue0w88bymn';
    public static $secret_key = 'skey_test_5x48st0dfumefhbbcog';
    public function __construct($money, $type, $night){
      $this->money = $money;
      $this->type = $type;
	  $this->night = $night;
    }

    public function showPayment($sql){
		if($_POST['type'] == 'Ballroom'){
			$this->classCss = "ballroompop";
			$this->pernight = "250,000";
			$this->perrsonMin = 2;
			$this->personMax = 100;

		}else if($_POST['type'] == 'Seminar'){
			$this->classCss = "seminarpop";
			$this->pernight = "50,000";
			$this->perrsonMin = 30;
			$this->personMax = 100;
		}else if($_POST['type'] == 'Deluxe Double Room'){
			$this->classCss = "doublepop";
			$this->pernight = "5,000";
			$this->perrsonMin = 1;
			$this->personMax = 2;
		}else if($_POST['type'] == 'Deluxe Twin Room'){
			$this->classCss = "twinpop";
			$this->pernight = "5,000";
			$this->perrsonMin = 1;
			$this->personMax = 4;
		}
		else if($_POST['type'] == 'Pool Villa'){
			$this->classCss = "poolpop";
			$this->pernight = "10,000";
			$this->perrsonMin = 1;
			$this->personMax = 4;
		}
	?>	
		<form method="POST" id="openPayment" action="../payment/vertify.php">
			<div class="creditpop overlayc">
				<a class="close" href="">&times;</a>
				<div class="model">
					<div class="<?php echo $this->classCss?>">
						<!-- // -->
					<div class="text-cover">
						<h1>HYGEAS HOTEL</h1>
						<h1>Khao Kho, Phetchabun</h1>
						<h1>Thailand</h1>
						<p class="price"><span><?php echo $this->pernight?> THB</span> / Night</p>
						<hr>
						<p><?php echo $_POST['type']." for ".$this->perrsonMin." - ".$this->personMax." person"?></p>
						<p><?php echo $_POST['checkin']?> to <?php echo $_POST['checkout']?> nights</p>
					</div>
					...
					<div class="receipt-box">
						...
						<table class="table">
						<tr>
							<td><?php echo $this->pernight." x ".$this->night?> </td>
							<td><?php echo $this->money?> THB</td>
						</tr>
						<tr>
							<td>Subtotal</td>
							<td id="cellSubtotal"><?php echo $this->money?> THB</td>
						</tr>
						<tfoot>
							<tr>
							<td>Sum</td>
							<td id="cellSum"><?php echo $this->money?> THB</td>
							</tr>
						</tfoot>
						</table>
						</div>
						<div class="payment-info">
							...
							<div class="form-container">
								<div class="field-container">
									...
									<input id="cardname" maxlength="20" type="text" name="card_name" >
								</div>
								<div class="field-container">
								...
								<div class="field-container">
									...
									<input  id="expiration" type="text" pattern="[0-9]*" name="expiration_month" inputmode="numeric"> /
									<input  id="expiration"  type="text" pattern="[0-9]*" name="expiration_year" inputmode="numeric">

								</div>
								<div class="field-container">
									<label for="securitycode">SECURITY CODE</label> 
									<input id="securitycode" type="text" pattern="[0-9]*" name="security_code" inputmode="numeric">
								</div>



								
                                        <label class="labelpomo" for="pomocode">Have a promo code? <i class="fa-solid fa-circle-info" style="color: #8f8f8f;"></i></label> 
                                        <input id="pomocode" type="text" name="event_code" >


								<!-- <button onclick="setDiscountandAmount()" class="btnpomo">Apply code</button> -->

          						<!-- Submit Button -->
								<input type="hidden" name="amount" value=<?php echo $this->money;?>>
								<input type="hidden" name="type" value=<?php echo $this->type;?>>
								<input type="hidden" name="people" value=<?php echo $_POST['people'];?>>
								<textarea name="command" class="hidee" rows="4" cols="50"><?php echo $sql;?></textarea>

								<button type="submit" class="btn"><?php echo "Price: ".$this->money. " THB";?></button>
							</div>
				   		</div>
					</div>
					</div>
	...
	<nav class="tabs-box">
		<div aria-label="simple tabs example" class="tabs-menu js-tabs-menu" role="tablist">
			<button class="tab-button js-tab-button" type="button" role="tab" name="room" onclick="hidedisplay('room')">
			<span class="tab-button__content">Room</span>
			</button>
			<button class="tab-button js-tab-button" type="button" role="tab" name="ballroom" onclick="hidedisplay('ballroom')">
			<span class="tab-button__content">Ballroom</span>
			</button>
			<button class="tab-button js-tab-button" type="button" role="tab" name="seminar" onclick="hidedisplay('seminar')">
			<span class="tab-button__content">Seminar</span>
			</button>
					...
					<div class="booking-form">
						...
						<form method="post" action="book.php">
							<div class="row">
								<div class="col-md-6">
									<div class="form-group">
										...
										<input class="form-controlday" name="checkin" type="date" min="<?php echo date('Y-m-d');?>"required>
								...
								<div class="col-md-6">
									<div class="form-group">
										...
										<input class="form-controlday" name="checkout" type="date" min="<?php echo date('Y-m-d',strtotime("+1 days"));?>"required>
									</div>
								</div>
							</div>
							<div class="row">
								<div class="col-md-6">
									<div class="form-group">
										...
										<select class="form-controlchild" name="people">
											<option>1</option>
											<option>2</option>
											<option>3</option>
                                            <option>4</option>
							...
							<div class="form-group">
								...
								<select class="form-controlroom" name="type"required>
						...
						<form method="post" action="book.php">
							...
							<div class="row">
								<div class="col-md-6">
									<div class="form-group">
										<span class="form-label">GUESTS&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
										<input type="number" class="inputpeople" placeholder="2" name="people" required>
										<span class="select-arrow"></span>
									</div>
								</div>
							</div>
							<input name="type" type="hidden" value="Ballroom">
							<div class="form-btn">
								<button class="submit-btn">Check availability</button>
							</div>
						</form>
						...
						<form method="post" action="book.php">
							...
							<div class="row">
								<div class="col-md-6">
									<div class="form-group">
										<span class="form-label">GUESTS&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
										<input type="number" class="inputpeople" placeholder="30" name="people" required>
										<span class="select-arrow"></span>
									</div>
								</div>
							</div>
    ...
    <script>
		...
		var SubmitError = document.getElementById('room');
